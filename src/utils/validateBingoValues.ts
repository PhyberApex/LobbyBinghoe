...
export default (bingoValues: Array<Array<boolean>>) : boolean => {
    ...
    for (let rowIndex = 0; rowIndex < bingoValues.length; rowIndex++) {
      ...
      for (let columnIndex = 0; columnIndex < bingoValues[rowIndex].length; columnIndex++) {
        // Check each row and column
        if (!bingoValues[rowIndex][columnIndex]) 
          bingoFoundInRow = false;
        if (!bingoValues[columnIndex][rowIndex]) 
          bingoFoundInColumn = false;
      }
  
          // Check diagonals
          if (!bingoValues[rowIndex][rowIndex]) 
          bingoFoundInDiag1 = false;
          if (!bingoValues[rowIndex][bingoValues.length - 1 - rowIndex]) 
          bingoFoundInDiag2 = false;
  
      // Check if any row or column has bingo
      if (bingoFoundInRow || bingoFoundInColumn) 
        return true;
    };
...
export function validateLobbyHoeBingo(bingoValues: Array<Array<boolean>>) : boolean {
  ...
  lobbyHoeSymbolLayout.forEach(coordinate => {
    if (!bingoValues[coordinate[0]][coordinate[1]])
    found = false;
  });
  return found;
};